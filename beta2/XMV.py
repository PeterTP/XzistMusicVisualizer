...
class XMV:
    def __init__(self):
        ...
        self.graph2d = Graph2D(self.win, self.PATH, self.playback.update_chunk_size)

        self.record = Record(self.win, self.PATH, self.playback.set_using_mic, self.graph2d.export)

        self.playback.playlist.setFont(QFont(self.fonts['nova_square'][0], 20))
        self.graph2d.graph.getAxis('bottom').setTickFont(QFont(self.fonts['nova_square'][0], 8))
        self.graph2d.graph.getAxis('left').setTickFont(QFont(self.fonts['nova_square'][0], 12))

        self.win.resizeDocks([self.graph2d.top_dock, self.graph2d.bottom_dock], [450, 600], Qt.Orientation.Vertical)
        self.win.tabifyDockWidget(self.graph2d.bottom_dock, self.record.dock)
        ...
        fps = win32api.EnumDisplaySettings(device.DeviceName, -1).DisplayFrequency
    ...
    def update_plots(self, indata, avg):
        self.graph2d.update_data(indata, avg)
